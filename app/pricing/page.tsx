...
import { Check, Crown, Zap, Calendar, MessageCircle, LogOut } from "lucide-react"
...
export default async function PricingPage() {
  ...
  const dbUser = await prisma.user.findUnique({
    where:  { id: user.id },
    select: { name: true, planStatus: true, trialEndsAt: true, planExpiresAt: true },
  })

  if (!dbUser) redirect("/login")

  // Se o plano ainda está ativo, manda de volta ao dashboard
  const now     = new Date()
  const expired =
    dbUser.planStatus === "EXPIRED" ||
    (dbUser.planStatus === "TRIAL"  && new Date(dbUser.trialEndsAt)  < now) ||
    (dbUser.planStatus === "ACTIVE" && dbUser.planExpiresAt !== null && new Date(dbUser.planExpiresAt) < now)

  if (!expired) redirect("/dashboard")

  const WHATSAPP_NUMBER = process.env.CONTACT_WHATSAPP ?? "5500000000000"
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-16">
      <div className="w-full max-w-3xl flex flex-col items-center gap-10">
              ...
              <CardContent className="flex flex-col gap-4">
                ...
                <a
                  href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
                    `Olá! Quero assinar o plano ${plan.name} do AgendaZap. Minha conta: ${dbUser.name}`
                  )}`}
                  target="_blank"
        ...
        <form action={signOut}>
          <button type="submit" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors underline underline-offset-2">
